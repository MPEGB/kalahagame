                    ...
                    <c:choose>
                        <c:when test="${game.player2.active}">
                            ...
                            <c:forEach var="i" begin="1" end="${rowLength - 1}" step="1" varStatus ="status">
                                <td>
                                    <a href="${pageContext.servletContext.contextPath}/move.do?pitId=${game.board.pits[totalPits - 1 - i].id}">${game.board.pits[totalPits - 1 - i].noOfseeds}</a>
                                </td>
                            </c:forEach>
                            <td rowspan="2">
                    ...
                    <c:choose>
                        <c:when test="${game.player1.active}">
                            <c:forEach var="i" begin="0" end="${rowLength - 2}" step="1" varStatus ="status">
                                <td>
                                    <a href="${pageContext.servletContext.contextPath}/move.do?pitId=${game.board.pits[i].id}">${game.board.pits[i].noOfseeds}</a>
                                </td>
                            </c:forEach>
                        </c:when>
                        <c:otherwise>
