package com.mpegb.game.kalaha.service;
...
/**
 * Represent kalaha game service
 * @author Bhushan Misha
 */
@Service("kalahaGameService")
@Transactional
public class KalahaGameService {
    ...
    public KalahaGame startTheGame(int clickedPitId, KalahaGame game) {
        ...
        while(i< KalahaGameEnum.NUMBER_OF_PITS.getValue() && seedsInClickedPit > 0) {
            if(seedsInClickedPit == 1) {
                ...
                if(isLastSeedInOwnEmptyPit(currentActivePlayer, pits[i])) {
                    ...
                    if(currentActivePlayer.getPlayerNumber() == 1) {
                        pits[KalahaGameEnum.PITS_FOR_PLAYER.getValue() - 1].setNoOfseeds(pits[KalahaGameEnum.PITS_FOR_PLAYER.getValue() - 1].getNoOfseeds() + numberOfSeedsToAdd);
                    } else {
                        pits[KalahaGameEnum.NUMBER_OF_PITS.getValue() - 1].setNoOfseeds(pits[KalahaGameEnum.NUMBER_OF_PITS.getValue() - 1].getNoOfseeds() + numberOfSeedsToAdd);
                    }
            ...
            if(i == KalahaGameEnum.NUMBER_OF_PITS.getValue()-1 && seedsInClickedPit > 0) {
                i = 0;
            } else {
                i++;
            }
    ...
    private KalahaGame gameOver(KalahaGame game, Pit[] pits, int player1NoOfSeeds, int player2NoOfSeeds) {
        Pit player1Kalaha = pits[KalahaGameEnum.PITS_FOR_PLAYER.getValue() - 1];
        Pit player2Kalaha = pits[KalahaGameEnum.NUMBER_OF_PITS.getValue() - 1];
    ...
    public boolean isOwnPit(Player player, int pitId) {
        return ((player.getPlayerNumber() == 1 && pitId < KalahaGameEnum.PITS_FOR_PLAYER.getValue()) ||
                (player.getPlayerNumber() == 2 && pitId > KalahaGameEnum.PITS_FOR_PLAYER.getValue()));
    }

    public Pit getOpositePit(KalahaGame game, int pitId) {
        return game.getBoard().getPitById(KalahaGameEnum.NUMBER_OF_PITS.getValue() - 2 - pitId);
    }
    ...
    public List<Pit> getPlayerPits(Board board, Player player) {
        ...
        if(player.getPlayerNumber() == 1) {
            for(int i=0; i<KalahaGameEnum.PITS_FOR_PLAYER.getValue()-2; i++) {
                pits.add(allPits[i]);
            }
        } else {
            for(int i=KalahaGameEnum.PITS_FOR_PLAYER.getValue(); i<KalahaGameEnum.NUMBER_OF_PITS.getValue()-1; i++) {
                pits.add(allPits[i]);
            }
        }
        return pits;
